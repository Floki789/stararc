...
interface WealthManagerAnimationProps {
  className?: string;
  isActive?: boolean;
}

const WealthManagerAnimation: React.FC<WealthManagerAnimationProps> = ({ className = "", isActive = true }) => {
    ...
    <div className={`relative h-[540px] overflow-visible ${className}`}>
      ...
      {isActive && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          >
            <div className="w-96 h-96 rounded-full bg-gradient-to-r from-amber-500/20 to-yellow-500/20 blur-3xl" />
          </motion.div>
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <span className="text-sm text-gray-400">{t('animations.wealthManager.features.control')}</span>
            </div>
            <div className="flex flex-col items-center">
              <Target className="w-8 h-8 text-yellow-400 mb-2" />
              <span className="text-sm text-gray-400">{t('animations.wealthManager.features.strategy')}</span>
            </div>
            <div className="flex flex-col items-center">
              <Crown className="w-8 h-8 text-amber-500 mb-2" />
              <span className="text-sm text-gray-400">{t('animations.wealthManager.features.independence')}</span>
            </div>
          </motion.div>
          ...
          <motion.button
            ...
            className="px-8 py-4 bg-gradient-to-r from-amber-600 to-yellow-600 text-white text-lg font-semibold rounded-lg transition-all duration-300 hover:shadow-xl cursor-pointer"
          >
...
export default WealthManagerAnimation;